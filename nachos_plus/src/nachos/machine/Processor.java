 ...
 *<tt>处理器</tt>类模拟支持r3000指令集子集的mips处理器。具体来说，
 * 处理器缺乏所有协处理器支持，只能在用户模式下执行。
 * 地址转换信息通过api访问。api还允许内核设置异常处理程序，以便在任何用户模式异常上调用。
 * <p>
 * The <tt>Processor</tt> API is re-entrant, so a single simulated processor
 * can be shared by multiple user threads.
 *
 * <TT>处理器</TT> API是可重入的，因此单个模拟处理器可以由多个用户线程共享。
 * <p>
 ...
 *
 * 处理器实例还包括用户程序可访问的物理内存页，其大小由构造函数确定。
 */

//指定计算机是否应提供MIPS处理器。在
//第一个项目，我们只运行内核代码，所以这是false的。在
//其他项目应该是true。
public final class Processor {
	...
	//调用异常处理程序时，将启用中断，CPU原因寄存器将指定异常的原因（请参阅<tt>exception<i>*</i></tt>）。
     ...
     */
    //获取异常处理程序，由最后一次调用<tt>setExceptionHandler()</tt>设置
     ...
     */
    //测试此处理器是使用软件管理的TLB，还是使用单级分页。 false为使用single-level paging
	//true为使用 software-managed TLB;
