...
//在nachos中，条件变量服从 <i>Mesa-style</i>语义。
// 当 <tt>wake()</tt> or <tt>wakeAll()</tt> 唤醒另一个线程时，
// 唤醒的线程被简单地放在就绪列表中，唤醒的线程有责任重新获取锁（这个重新获取是在 <tt>sleep()</tt>中获取的核心）。

//相比之下，一些条件变量的实现遵循 <i>Hoare-style</i>语义，其中调用<tt>wake（）</tt>的线程将锁和cpu交给唤醒线程，
// 唤醒线程立即运行，并在唤醒线程退出临界区时将锁和cpu交还给唤醒程序.

//使用mesa样式语义的结果是，在唤醒的线程有机会运行之前，其他线程可以获取锁并更改数据结构。mesa风格语义的进步在于它更容易实现。
